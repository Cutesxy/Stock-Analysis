...
一个本地运行的个人投资决策工具:**月线滚动通道**定位买卖点、**历史锚点回测**量化胜率、**本地数据库**管理持仓。前后端分离(FastAPI + 原生ES模块),APP式界面——手机浏览器打开可"添加到主屏幕"当独立应用用。
...
**四个页面**(桌面顶部导航/手机底部Tab):
- **概览**——三张决策卡(σ仪表盘/动作距离/一键记账)+ 通道图表 + 待挂条件单 + 时间窗口
- **持仓**——仓位卡(浮盈/加权成本)+ 配置结构对比 + 账户收益曲线(流水逐日回放)+ 交易流水维护
- **量化分析**——系统层/资产层/情境层胜率、σ分区查表、五种加仓方式对比、追涨事件检验
